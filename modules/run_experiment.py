...
def generateEnsembleIntegration(cm_args, batch, start_time, end_time, dump_time, split_files='neither', move_for_assim=True):
    ...
    if cm_args.algorithm == 'ensrf':
        command = [ 
            "%s $base/arps %s > %s " % (batch.getMPIprogram(), arps_input_file_name, arps_debug_file_name),
            "rm %s" % ( " ".join(extraneous_files) )
        ]
        command.extend(epilogue)
    elif cm_args.algorithm == '4densrf':
        command = [ 
            "%s $base/arps %s %s > %s " % (batch.getMPIprogram(), arps_input_file_name, arpsenkf_input_file_name, arps_debug_file_name),
            "rm %s" % ( " ".join(extraneous_files) )
        ]
        command.extend(epilogue)
...
def generateEnsemblePerturbations(cm_args, batch, start_time):
    ...
    command = "%s -n 1 -o 0 $base/arpsenkfic %s < %s > %s" % \
...
def generateEnKFAssimilation(cm_args, batch, assim_time, radar_data_flag):
    ...
    command_lines = [
        "cd %s" % work_path,
        "%s $base/arpsenkf %s < %s > %s" % (batch.getMPIprogram(), arps_input_file_name, enkf_input_file_name, enkf_debug_file_name),
        "cd -",
        "",
    ]
...
def generateDomainSubset(cm_args, batch, src_path, start_time, end_time, step_time, perturb_ic=True, copy_ic=False):
    ...
    if perturb_ic:
        commands.append("%s -n 1 -o 0 $base/arpsintrp %s/arps.input < %s > %s ; %s -n 1 -o 0 $base/arpsenkfic %s < %s > %s" % (batch.getMPIprogram(), input_path, interp_input_file_name, 
            interp_debug_file_name, batch.getMPIprogram(), arps_input_file_name, arpsenkfic_input_file_name, arpsenkfic_debug_file_name))
    else:
        commands.append("%s -n 1 -o 0 $base/arpsintrp %s/arps.input < %s > %s" % (batch.getMPIprogram(), input_path, interp_input_file_name, interp_debug_file_name))
        if copy_ic:
            commands.extend([
...
def submit(cm_args, batch, command_lines, wall_time, n_cores, hybrid=False):
    job_suffixes = []
    job_completed = []
    ...
    for key, commands in command_lines.iteritems():
        ...
        file_text = batch.gen(commands, jobname=job_key, debugfile="%s/%s.output" % (debug_path, job_key), nmpi=n_mpi, nnodes=n_nodes, queue='normal', timereq=wall_time + ":00")
    ...
    while True:
        ...
        if len(queue) > 0:
            ...
            for idx in xrange(len(job_completed)):
                job_completed[idx] = True
...
#       args.radar_data_flags = radar_data_flag
    ...
    if args.free_forecast:
        ...
        for t_ens in xrange(args.t_ens_start, args.t_ens_end, args.dt_assim_step):
            ...
            submit(args, batch, assimilation_lines, req_time, nproc_x * nproc_y * batch.getNCoresPerNode(), hybrid=True)
