...
def generateEnsembleIntegration(cm_args, batch, start_time, end_time, dump_time, split_files='neither', move_for_assim=True):
        ...
        if dump_split:
            epilogue = doForMPIConfig(
                "mv %s/%s/%s.hdf%06d_%s %s/%s/%s.hdf%06d_%s" % \
                    (work_path, 'EN%(ens)s', 'ena%(ens)s', end_time, '%(x_proc)03d%(y_proc)03d', work_path, 'ENF%(ens)s', 'enf%(ens)s', end_time, '%(x_proc)03d%(y_proc)03d'), cm_args.mpi_config_dump
            )
        else:
            epilogue = [
    ...
    for n_ens in cm_args.members:
        ...
        editNamelistFile("%s/%s" % (cm_args.base_path, cm_args.arps_template), ens_arps_file_name,
            nproc_x=nproc_x, nproc_y=nproc_y, nproc_x_out=nproc_x_dump, nproc_y_out=nproc_y_dump,
...
def submit(cm_args, batch, command_lines, wall_time, n_cores, start_time, end_time, hybrid=False, squash_jobs=False):
    ...
    for key, commands in command_lines.items():
        ...
        if(envname == 'rice'):
            ...
            queuename = 'normal'
            file_text = batch.gen(commands, jobname=job_key, debugfile="%s/%s.output" % (debug_path, job_key), nmpi=n_mpi, nnodes=n_nodes, queue=queuename, timereq=wall_time + ":00")
...
def main():
    ...
    ap.add_argument('--mpi-config-model', dest='mpi_config_model', nargs=2, default=(3, 4), type=int)
...
#   boundary_path = "%s/boundary/" % args.base_path
    ...
    work_path, input_path, debug_path, batch_path, boundary_path, hx_path =\
        getPaths(args.base_path, args.job_name, work=True, input=True, debug=True, batch=True, boundary=True, hx=True)
    ...
    if member_list == []: member_list = list(range(args.n_ens_members))
    args.members = member_list

    nproc_x_mod, nproc_y_mod = args.mpi_config_model
    args.mpi_config_dump = args.mpi_config_dump or arg.mpi_config_model
    ...
    editNamelistFile("%s/%s" % (args.base_path, args.arps_template), "%s/arps.input" % input_path,
        nproc_x=nproc_x_mod, nproc_y=nproc_y_mod, nproc_x_out=nproc_x_dump, nproc_y_out=nproc_y_dump,
        dmp_out_joined=joined,
        inisplited=3 * (1 - joined),
        sfcdat=3,
        dirname=work_path
...
#       args.radar_data_flags = radar_data_flag
    ...
    if args.restart:
        for t_ens in range(args.t_ens_start, args.t_ens_end + args.dt_assim_step, args.dt_assim_step):
